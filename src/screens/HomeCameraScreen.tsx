...
import { hapticCapture, hapticStop, notifyError, playShutter } from '../services/feedback';
...
export default function HomeCameraScreen({ navigation }: HomeCameraScreenProps) {
  ...
  const hasResult = lastImage !== null && lastDescription !== null && phase === 'idle';
  ...
  const debouncedCapture = useDebounceCallback(
    useCallback(
      (...args: unknown[]) => {
        ...
        if (trigger === 'voice' || trigger === 'volume' || trigger === 'button') {
  ...
  const pickImage = useCallback(async (): Promise<void> => {
    ...
    try {
      const options = { mediaTypes: ['images' as const], quality: GALLERY_IMAGE_QUALITY };
  ...
  const handleVolumePress = useCallback(() => captureFlow('volume'), [captureFlow]);
  ...
  const openQA = useCallback(() => {
    ...
    navigation.navigate('QASession', { image: lastImage, caption: lastDescription });
  ...
  const openObstacle = useCallback(() => navigation.navigate('ObstacleMode'), [navigation]);
  const stopSpeaking = useCallback(() => {
    tts.stop();
    void hapticStop();
  }, []);

  useVolumeButtonPTT(handleVolumePress, { enabled: !hasResult && phase === 'idle' });
  useVoiceControl({
  ...
  if (hasResult && lastImage !== null && lastDescription !== null) {
    const pad = { paddingTop: insets.top + spacing.md, paddingBottom: insets.bottom + spacing.md };
    return (
      <View style={[styles.resultContainer, pad]}>
        <CaptionPanel imageUri={lastImage.uri} caption={lastDescription} isLoading={false} />
        <View style={styles.actions}>
          <BigActionButton label={HOME.ASK_ABOUT_IMAGE} onPress={openQA}
            accessibilityLabel={HOME.ASK_ABOUT_IMAGE} accessibilityHint={HOME.ASK_HINT} />
          <BigActionButton label={HOME.NEW_CAPTURE} onPress={handleNewCapture}
            variant="secondary" compact accessibilityLabel={HOME.NEW_CAPTURE} />
          <BigActionButton label={HOME.READ_AGAIN} onPress={repeatDescription}
            variant="secondary" compact accessibilityLabel={HOME.READ_AGAIN} />
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <CornerControl position="topLeft" iconName="settings-outline" onPress={openSettings}
        label={SCREEN_TITLES.SETTINGS} hint={HOME.SETTINGS_HINT} />
      <CornerControl position="topRight" iconName="walk-outline" onPress={openObstacle}
        label={SCREEN_TITLES.OBSTACLE} hint={HOME.OBSTACLE_HINT} />
      <CornerControl position="bottomLeft" iconName="images-outline" onPress={handleGallery}
        label={HOME.GALLERY_LABEL} hint={HOME.GALLERY_HINT} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  bootText: { ...typography.body, color: colors.textMuted, textAlign: 'center' },
  scrimTop: { ...scrimBase, top: 0, height: TOP_SCRIM_HEIGHT, backgroundColor: colors.scrimTop },
  scrimBottom: {
    ...scrimBase,
    bottom: 0,
  ...
  shutterWrap: { position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'center' },
  resultContainer: {
    flex: 1,
    backgroundColor: colors.bg,
